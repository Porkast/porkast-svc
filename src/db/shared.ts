...
export const generateListenLaterRSSXml = async (db: DbClient, userRef: string): Promise<string> => {
  ...
  const feed = new Podcast({
    ...
    feedUrl: `${RSS_BASE_URL}/listenlater/${userRefOf(user)}`,
...
export const generatePlaylistRSSXml = async (db: DbClient, userRef: string, playlistId: string): Promise<string> => {
  const userInfo = await getUserRowByRef(db, userRef)
  ...
  const feed = new Podcast({
    ...
    feedUrl: `${RSS_BASE_URL}/playlist/${playlistId}/${userRefOf(user)}`,
    siteUrl: 'https://porkast.com',
    imageUrl: 'https://porkast.com/porkast-text-logo.png',
    author: nickname,
    managingEditor: nickname,
...
export const generateSubscriptionRSS = async (db: DbClient, userRef: string, keyword: string): Promise<string> => {
  const userInfo = await getUserRowByRef(db, userRef)
  ...
  const feed = new Podcast({
    ...
    feedUrl: `${RSS_BASE_URL}/subscription/${userRefOf(user)}/${encodeURIComponent(subsciptionInfo.Keyword)}`,
    siteUrl: 'https://porkast.com',
    imageUrl: 'https://porkast.com/porkast-text-logo.png',
    author: nickname,
    managingEditor: nickname,
